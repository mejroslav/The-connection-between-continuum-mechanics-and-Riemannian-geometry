...
\begin{proof}[Proof of theorem \ref{theorem1:ekvivalence}]
    ...
    where
    \begin{align}
        E^{ijk}(x) =& (b^{ij}_m b^{mk}_{p,s} + b^{ij}_{s,m} b^{mk} ) u^p_x u^s_x + b^{ij}_m b^{mk}_p u^p_{xx} \\
        F^{ijk}(x) =& (b^{ij}_{s,m} g^{mk} + g^{mk}_{,s} b^{ij}_m + b^{ij}_m b^{mk}_s)u_x^s \\
        G^{ijk}(x) =& g^{ij}_{,m} b^{mk}_s u^s_x \\
        H^{ijk}(x) =& g^{ij}_{,m} g^{mk} \\
        I^{ijk}(x) =& b^{ij}_m g^{mk} \:.
    \end{align}
    ...
    Other terms in $\mathcal J^{ijk}(x,y,z)$ are handled in the similar way. We are insterested only in functions $A^{ijk}_{00}$ and $A^{ijk}_{02}$. We shall also use the compatibility condition \eqref{eq:Th.Novikov-konexe}. 
